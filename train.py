from processor.ctc_exp import ctc_vit_model
...
from loss.build_loss import build_loss,build_seg_loss
import random
import torch
import numpy as np
import os
...
if __name__ == '__main__':
    ...
    if len( cfg.DATASETS.TRAIN )==1:
        nb_domain= cam_dict[cfg.DATASETS.TRAIN[0]]
    else:
        nb_domain= len( cfg.DATASETS.TRAIN )
    print(f'====================={nb_domain}')
    model_name = cfg.MODEL.NAME
    model = make_model(cfg, modelname=model_name, num_class=num_classes, camera_num=None, view_num=None,nb_domain=nb_domain)
    ...
    loss_func, center_cri = build_loss(cfg, num_classes=num_classes,nb_domain=nb_domain)
    seg_loss_fn=build_seg_loss(cfg)
    optimizer = make_optimizer(cfg, model)
    scheduler = create_scheduler(cfg, optimizer)
    
    ################## patch loss ####################
    patch_centers = Patchloss.PatchMemory(momentum=0.1, num=1)
    pc_criterion = Patchloss.Pedal(scale=cfg.MODEL.PC_SCALE, k=cfg.MODEL.CLUSTER_K)
    ...
    # }    
    do_train_dict = {
        'ctc_vit':  ctc_vit_model
    }
    
    if model_name not in do_train_dict.keys():
        print(f'use {model_name} to train')
